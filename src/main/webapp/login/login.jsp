...
<%
    ...
    if(request.getParameter("btn_login")!=null) //check condition button login click event not null "btn_login"
    {
        ...
        email=request.getParameter("name"); //textbox name "txt_email"
        ...
        String dburl="jdbc:mysql://localhost:3306/real_estate"; //database url string the "jsp_multiuser_login_db" is database name
        String dbuname="root"; //database username
        String dbpwd ="password"; //database password
        ...
        try
        {
            ...
            pstmt=con.prepareStatement("SELECT * FROM tbl_user WHERE name=? AND password=? AND role=? "); //sql select query
            ...
            if(rs.next())
            {
                ...
                if(email.equals(dbname) && password.equals(dbpassword) && role.equals(dbrole)) //check fethable database record and user input value are match after continue
                {
                    if(dbrole.equals("admin")) //check dbrole is "admin" continue innner in if condition
                    {
                        session.setAttribute("admin_login",dbname); //session name is "admin_login" and store fetchable database "dbemail" address
                        response.sendRedirect("admin/main.jsp"); //after login success redirect to "admin_home.jsp" page in admin folder
                    }

                    else if(dbrole.equals("user")) //check dbrole is "user" continue innner in else if condition
                    {
                        session.setAttribute("user_login",dbname); //session name is "user_login" and store fetchable database "dbemail" address
